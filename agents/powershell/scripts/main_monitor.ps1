...
# ============================================================================
param(
    [switch]$SkipElevation  # Skip elevation check (when already running as admin)
)
...
if (-not $isAdmin -and -not $SkipElevation) {
    ...
    try {
        ...
        Write-Host "Failed to elevate to administrator: $_" -ForegroundColor Red
        Write-Host "Please run PowerShell as Administrator manually." -ForegroundColor Red
        exit 1
    }
}
...
if ($isAdmin) {
    ...
    Write-Host "Warning: Running without administrator privileges - some metrics may be unavailable" -ForegroundColor Yellow
}
Write-Host ""
