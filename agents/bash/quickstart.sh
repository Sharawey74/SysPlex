...
run_once() {
    ...
    echo ""
    chmod +x "${HOST_ROOT}/monitors/main_monitor.sh"
    bash "${HOST_ROOT}/monitors/main_monitor.sh"
    echo ""
    echo "${GREEN}✅ Done! Output at: Host/../../data/metrics/latest.json${NC}"
}
...
view_metrics() {
    echo ""
    if [ ! -f "${HOST_ROOT}/../../data/metrics/latest.json" ]; then
    ...
    if command -v jq >/dev/null 2>&1; then
        jq -C . "${HOST_ROOT}/../../data/metrics/latest.json" | less -R
    else
        cat "${HOST_ROOT}/../../data/metrics/latest.json" | less
    fi
}
